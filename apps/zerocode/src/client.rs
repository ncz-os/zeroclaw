...
impl RpcClient {
    ...
    /// organization row. NVIDIA builds populate it via `nvzoder sync`.
...
/// the daemon's `org_cost.json`. Mirrors the NVIDIA Supernova cache shape but is
/// vendor-neutral here; absent on vanilla builds.
#[derive(Debug, Clone, Default, serde::Deserialize)]
pub struct OrgCost {
    #[serde(default)]
    pub year: i32,
    ...
    /// Display label for the organization scope (e.g. "NVIDIA"). Falls back to
