...
pub(crate) struct Dashboard {
    ...
    /// account, fetched on the Cost tab so the dashboard mirrors the
    /// `nvzoder/zoder report` period breakdown. `(label, summary)`.
    cost_periods: Vec<(String, CostSummaryResult)>,
    /// Optional org-level billed snapshot (`cost/org`). Present only when the
    /// daemon has an `org_cost.json` (NVIDIA builds via `nvzoder sync`); `None`
...
impl Dashboard {
    ...
    async fn poll_data(&mut self) {
        ...
        match self.tab {
            ...
            Tab::Cost => {
                ...
                // mirroring `nvzoder/zoder report`. Best-effort per window.
                ...
                // Org-level billed snapshot (present only on NVIDIA builds that
                // run `nvzoder sync`). Best-effort; absence is normal.
    ...
    fn draw_cost(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        // account — mirrors `nvzoder/zoder report`. Paid vs free tokens make the
        ...
        // Organization-level billed snapshot (NVIDIA builds only — present when
        // `nvzoder sync` has written org_cost.json). Shows billed YTD + a simple
...
/// time, period-to-date. Mirrors `nvzoder/zoder report` so the dashboard and
/// CLI agree on what each period means. The daemon parses these bounds and
/// converts to UTC.
