...
fn resolve_rates(pricing: &HashMap<String, f64>, model: &str) -> (f64, f64, f64) {
    ...
    // No hand-priced `cost.rates` entry for this model. Return zero here; the
    // caller (`record_tool_loop_cost_usage`) then consults the daemon-wide
    // pricing catalog via `pricing_catalog::global_pricing_rates` before billing.
    (0.0, 0.0, 0.0)
}
