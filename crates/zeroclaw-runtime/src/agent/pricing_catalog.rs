...
//! daemon:
//!   * **zoder** writes it from the public LiteLLM / OpenRouter feed
//!     (`zoder pricing refresh`, daily via launchd) — its only pricing source.
//!   * **nvzoder** is Supernova-authoritative and writes NO file by default; it
//!     only populates the external providers' rates on explicit opt-in
//!     (`nvzoder pricing refresh --external`).
//!
//! When the file is present this makes `cost_usd` non-zero for cloud models the
//! operator never hand-priced in `config.toml`. When absent (nvzoder default)
//! the engine prices nothing here and EIH stays `$0`.
//!
//! Matching is intentionally **exact and case-insensitive on the full model id
//! only** — no leaf/substring fuzzy. EIH / NVCF ids (`nvcf/...`, `nvidia/...`)
//! are absent from the public catalog, so an exact-only match leaves them at
//! `$0` (correct: they are not billed to the department). A broad fuzzy match
//! would misprice `gpt-oss-120b` running free on EIH against its public rate,
//! or collapse `grok-4.3` onto `grok-4`; exact matching avoids both.
...
/// written by the nvzoder/zoder pricing feed.
...
impl GlobalPricingCatalog {
    ...
    /// the catalog (which keeps EIH/NVCF ids at `$0`).
...
mod tests {
    ...
    fn exact_case_insensitive_match_resolves_component_rates() {
        ...
        assert_eq!(c.rates_for("gemini-3.1-pro-preview"), Some((1.25, 10.0, 0.0)));
        assert_eq!(c.rates_for("Gemini-3.1-Pro-Preview"), Some((1.25, 10.0, 0.0)));
    }
    ...
    #[test]
    fn eih_ids_absent_from_catalog_stay_unpriced() {
        // Public catalog prices a bare `gpt-oss-120b`, but the EIH provider
        // serves it under a provider-qualified id — exact-only matching must
        // NOT misprice the free EIH variant.
        let c = catalog_from(r#"{"models":{"gpt-oss-120b":{"output_usd_per_mtok":0.5}}}"#);
        assert_eq!(c.rates_for("nvcf/gpt-oss-120b"), None);
        assert_eq!(c.rates_for("nvidia/gpt-oss-120b"), None);
    }
    ...
    fn no_fuzzy_substring_or_leaf_collapse() {
        ...
        assert_eq!(c.rates_for("grok-4.3"), None, "must not collapse grok-4.3 onto grok-4");
    }
}
