...
pub async fn run(
    ...
    // This is consumption only and vendor-neutral: zoder populates the file from
    // the public feed; nvzoder (Supernova-authoritative) has no file by default,
    // so EIH stays $0. Refreshing the file is a CLI + launchd concern, never a
    // public-feed fetch inside this shared daemon.
