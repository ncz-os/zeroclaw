...
impl RpcDispatcher {
    ...
    fn handle_cost_query(&self, params: &Value) -> RpcResult {
        ...
        // Reports view, `nvzoder/zoder report`) pull day/month/quarter/YTD
    ...
    /// presence-gated: an NVIDIA build's `sync` writes this file from its
        ...
        match std::fs::read_to_string(&path) {
            Ok(raw) => {
                let value: Value = serde_json::from_str(&raw).map_err(|e| {
                    rpc_err(INTERNAL_ERROR, format!("org_cost.json is not valid JSON: {e}"))
                })?;
                Ok(value)
            }
            Err(_) => Ok(Value::Null),
